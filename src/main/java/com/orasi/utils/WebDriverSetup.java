...
public class WebDriverSetup {
	...
	public WebDriverSetup(String application, String browserUnderTest, String browserVersion, String operatingSystem,
		...
		if (getRunLocation().equalsIgnoreCase("local")) {
			...
			case "linuxos":
				if (getBrowserUnderTest().equalsIgnoreCase("Firefox") || getBrowserUnderTest().equalsIgnoreCase("FF")) {
					...
					file = new File(this.getClass().getResource(Constants.DRIVERS_PATH_LOCAL + "/linux/chromedriver")
							.getPath());
					System.setProperty("webdriver.chrome.driver", file.getAbsolutePath());
					try {
						// Ensure the permission on the driver include
		...
		case "android":
			TestReporter.assertTrue(
					platform.trim().replace(" ", "")
							.equalsIgnoreCase(operatingSystem.toString().toLowerCase().trim().replace(" ", "")),
					"The System OS [" + platform.trim().replace(" ", "")
							+ "] did not match that which was passed in the TestNG XML ["
							+ operatingSystem.toString().toLowerCase().trim().replace(" ", "") + "].");
			break;
		default:
			...
			Assert.assertTrue(osFound, "The System OS [" + platform
					+ "] did not match that which was passed in the TestNG XML [" + operatingSystem + "].");
			break;
		}
	}
	...
	public void linuxFirefoxSetup() throws IOException, InterruptedException {
		...
		Assert.assertEquals(browserVersion.isEmpty(), false,
		...
		String binaryMozillaUrl = "https://ftp.mozilla.org/pub/mozilla.org/firefox/releases/" + firefoxVersion
				+ "/linux-x86_64/en-US/firefox-" + firefoxVersion + ".tar.bz2";
		...
		if (!linuxFirefoxfindBinaries(homeDirectory, binaryLocation)) {
			...
			Assert.assertEquals(checkShellProcessForErrors(proc), 0, "Invoking wget on the URL [" + binaryMozillaUrl
			...
			proc = Runtime.getRuntime()
					.exec(new String[] { bashLocation, bashArguments, "bunzip2 " + binaryContainer });
			proc.waitFor();
			System.out.println(".......done");
			Assert.assertEquals(checkShellProcessForErrors(proc), 0,
					"An error was encountered while unzipping the file [" + binaryContainer + "].");
			...
			System.out.println(".......done");
			Assert.assertEquals(checkShellProcessForErrors(proc), 0,
					"An error was encountered while decompressing the tarball [" + binaryTarball + "].");

			// Move the file to the preferred location
			System.out.print("Moving the directory: " + binaryDirectory);
			proc = Runtime.getRuntime()
					.exec(new String[] { bashLocation, bashArguments, "mv firefox " + homeDirectory + "/" + username });
			proc.waitFor();
			System.out.println(".......done");
			Assert.assertEquals(checkShellProcessForErrors(proc), 0,
					"An error was encountered moving the file from the current working directory to [" + homeDirectory
							+ "/" + username + "].");

			// Verify that the binaries are now found
			System.out.print("Verifying binaries");
			Assert.assertTrue(linuxFirefoxfindBinaries(homeDirectory, binaryLocation),
	...
	private boolean linuxFirefoxfindBinaries(String homeDirectory, String binaryLocation) {
		...
		Assert.assertTrue(file.isDirectory(),
				"An object with the path '" + homeDirectory + "' was found to exist but is not a directory.");
		/*
