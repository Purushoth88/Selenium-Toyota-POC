...
public final class PrettyPrinter {
	...
	private static <T> String printPreparedTable(T[][] table, int widths[], String horizontalBorder,
			Printer<T> printer) {
	...
	private static <T> String getRow(T[] row, int[] widths, int lineLength, Printer<T> printer) {
		...
		for (int i = 0; i < maxWidths; i++) {
			builder.append(padRight(getCellValue(safeGet(row, i, printer), printer), widths[i]))
