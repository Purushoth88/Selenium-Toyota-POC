...
public class WebtableImpl extends ElementImpl implements Webtable {
		...
		for (WebElement rowElement : rowCollection) {
			if (startRow > currentRow) {
				...
				if (currentRow <= rowCollection.size()) {
					...
					te.getDriver().manage().timeouts().implicitlyWait(TestEnvironment.getDefaultTestTimeout(),
							TimeUnit.SECONDS);
	...
	public WebElement getCell(WebDriver driver, int row, int column) {
		...
		Assert.assertEquals(Boolean.valueOf(found), Boolean.TRUE,
				"No cell was found for row [" + String.valueOf(row) + "] and column [" + String.valueOf(column)
						+ "]. The column count for row [" + String.valueOf(row) + "] is ["
						+ String.valueOf(columnCollection.size()) + "]");
	...
	public int getColumnCount(WebDriver driver, int row) {
		...
		Assert.assertEquals(rowFound, true,
				"The expected row [" + String.valueOf(row)
						+ "] was not found. The number of rows found for webtable [ <b>@FindBy: "
						+ getElementLocatorInfo() + " </b>] is [" + String.valueOf(rowCollection.size()) + "].");
	...
	public String getCellData(WebDriver driver, int row, int column) {
		driver.manage().timeouts().implicitlyWait(0, TimeUnit.SECONDS);
		String cellData = "";
		cellData = this.element.findElement(By.xpath("tbody/tr[" + row + "]/td[" + column + "]|tbody/tr[" + row
				+ "]/th[" + column + "]|tr[" + row + "]/td[" + column + "]|tr[" + row + "]/th[" + column + "]"))
				.getText();
		driver.manage().timeouts().implicitlyWait(TestEnvironment.getDefaultTestTimeout(), TimeUnit.SECONDS);
		return cellData;
	}
		...
		for (WebElement rowElement : rowCollection) {
			if (startRow > currentRow) {
				...
				if (currentRow <= rowCollection.size()) {
					...
					driver.manage().timeouts().implicitlyWait(TestEnvironment.getDefaultTestTimeout(),
							TimeUnit.SECONDS);
	...
	public int getColumnWithCellText(WebDriver driver, String text, int rowPosition) {
		...
		Assert.assertEquals(Boolean.valueOf(found), Boolean.TRUE,
				"No cell in row [" + String.valueOf(rowPosition) + "] was found to have the text [" + text + "].");
		return columnFound;
	}
