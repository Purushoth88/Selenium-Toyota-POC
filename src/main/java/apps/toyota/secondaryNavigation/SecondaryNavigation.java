...
/**
 * @summary Contains the methods & objects for the Toyota.com Secondary
 *          Navigation bar
...
public class SecondaryNavigation extends com.orasi.utils.TestEnvironment {
	...
	private void clickFindADealer() {
		List<WebElement> list = driver
				.findElements(By.xpath("//*[@id=\"tcom-nav-zip-flyout\"]/div/div/div[2]/div/input"));

		TestReporter.log("Click 'Find A Dealer'");
		/*
		 * //Grab the number of links on the current page List<WebElement>
		 * myList = getDriver().findElements(By.tagName("a")); int listSize =
		 * myList.size(); TestReporter.log("Number of Inputs = " +
		 * String.valueOf(listSize)); for(int i = 0; i < listSize; i++){
		 * TestReporter.log("Input " + String.valueOf(i)); Element ele = new
		 * ElementImpl(myList.get(i)); ele.jsClick(getDriver()); }
		 */

		// Click the link
		lnkFindADealer.jsClick(getDriver());

		// Determine if OS is Windows 7. If so, apply a wait to let the page
		// load
		if (isWindows7()) {
			windows7Wait();
		}

		// Loop until the number of links changes, thereby indicating that a new
		// page was loaded
		loopCounter = 0;
